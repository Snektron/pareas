...
let compactify [n] (parents: [n]i32): [n]i32 =
    -- TODO: Make sure that deleted nodes are all marked as -1.
    let depths = compute_depths parents
    -- Add one extra so we can be sure that -1 depths are sorted last.
    -- TODO: Consider a partition here?
    let max_depth = 2 + reduce i32.max (-1) depths
